...
from flaskr.auths.utils import save_picture
...
def score_upload():
    ...
    for student in students:
        score = request.form[str(student.id)]
        if score:
            score = str(score)
            for result in results:
                student_info_exist = False
                if (
                    ...
                    total_score = (
                        0
                        if result.total_score == "" or result.total_score == None
                        else total_score
                    )
...
def result_download(student_id):
    ...
    try:
        ...
        # Serve the file
        return send_file(
            output_path, as_attachment=True, download_name="student_report_card.docx"
        )
